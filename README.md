...
클라이언트에서 서버에게 HTTP 요청을 보내면 서버는 현재 수행되고 있는 환경의 디렉토리 구조 및 파일 시스템을 `HTML 문서` 로 만들어서 클라이언트에게 제공 해줍니다.
...
우리의 웹 서버 프로그램은 `ls` 명령어를 통해 디렉토리 구조를 파악하며

이를 `HTML5` 문서로 생성해서 클라이언트에게 전송합니다.

여기서 서버 프로그램이 실행하는 `ls` 명령어는 OS 에서 제공해주는 여러 시스템 콜을 활용해서 

모듈을 직접 구현한 뒤 호출하는 방식으로 진행했습니다.

### ls 모듈 지원 범위

실제 리눅스에서 제공하는 `ls 명령어` 와 동일한 인터페이스로 사용할 수 있도록 개발했습니다.
...
이 신호에 신호 처리부를 등록해서 자식 프로세스의 종지 상태를 회수하고 좀비 프로세스가 되는 것을 방지합니다.

또한 주기적으로 로그를 출력 해주기 위해 `SIGALRM` 신호를 사용하며 마찬가지로 신호 처리부에서 로그를 생성하는 로직을 수행합니다.
...
만약 `MinIdleNum > idle process` 혹은 `idle process > MaxIdleNum` 이 된다면 현재 자식 프로세스를 생성 또는 종료하여 5개를 유지하도록 합니다.
